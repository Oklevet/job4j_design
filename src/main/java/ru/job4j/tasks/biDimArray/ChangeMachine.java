...
public class ChangeMachine {
    ...
    public static List<int[]> calcByHead(int[] nums, int startId, int[] res) {
        ...
        int prevStartId = startId;
        int currSum = amountSum - Arrays.stream(res).sum();

        while (startId < nums.length){  // || result[result.length - 1] < currSum / nums[3]) {
            System.out.println("startId = " + startId + ";  nums.length = " + nums.length);
            prevStartId = startId;
            ...
            System.out.println("prevStartId = " + prevStartId + ";  nums.length = " + nums.length);
            if (prevStartId < nums.length - 1) {
                System.out.println(2);
                vars.addAll(calcByHead(nums, ++prevStartId, result));
            }
            System.out.println("prevStartId = " + prevStartId + ";  startId = " + startId);
            if (startId == nums.length - 1) {
                return vars;
            }
        }
        //vars.add(new int[] {n1, n2, n3, n4});
        return vars;
    }
